...
def preprocess_data(data):
    ...
    combined_data = combined_data.dropna()  # Drop rows with any remaining NaN values
    return combined_data
...
def feature_engineering(data):
    ...
    # Add a time decay weight
    if 'Date' in data.columns:
        data['Date'] = pd.to_datetime(data['Date'])
        max_date = data['Date'].max()
        data['Time_Weight'] = data['Date'].apply(lambda x: np.exp(-(max_date - x).days / 365))
    else:
        data['Time_Weight'] = 1  # Default to no time decay if no date column exists
...
def visualize_logistic_regression(model, X_test, y_test, results):
    """
    Generate a series of visualizations for logistic regression performance.
    """
    # 1. Plot Feature Importance (Model Coefficients)
    plt.figure(figsize=(10, 6))
    feature_importance = pd.Series(model.coef_[0], index=X_test.columns)
    ...
    plt.figure(figsize=(10, 6))
    plt.hist(results['Win_Probability'], bins=20, color='skyblue', edgecolor='black')
    ...
    # 3. Predicted vs Actual Outcomes
    ...
    for (i, j), val in np.ndenumerate(cm):
        ax.text(j, i, f"{val} ({cm_normalized[i, j]*100:.1f}%)", ha="center", va="center", color="red")
